      ...
      <Card className="bg-[#262626] border-[#343434]">
        ...
        <CardContent>
          <div className="text-center py-12">
            <Image className="w-16 h-16 text-[#B0AFAF] mx-auto mb-4 opacity-50" />
            <p className="text-[#B0AFAF] text-lg">No media available</p>
    ...
    <>
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {media.map((item, index) => (
              ...
              >
                <div className="border-2 border-[#343434] rounded-xl bg-[#1A1A1A] overflow-hidden hover:border-primary/30 transition-all duration-300 group-hover:shadow-lg group-hover:shadow-primary/10">
                  {/* Media Thumbnail */}
                  <div className="aspect-video bg-[#262626] relative overflow-hidden">
                    {item.media_type === "video" ? (
                      <>
                        <div className="w-full h-full bg-linear-to-br from-[#1A1A1A] to-[#262626] flex items-center justify-center">
                          <Video className="w-12 h-12 text-[#B0AFAF] opacity-50" />
                        </div>
                        <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                          <div className="w-16 h-16 bg-primary/90 rounded-full flex items-center justify-center shadow-lg">
                            <Play className="w-6 h-6 text-[#0F0F0E] ml-1" />
                          </div>
                        </div>
                      </>
                      ...
                      <div className="w-full h-full bg-linear-to-br from-[#1A1A1A] to-[#262626] flex items-center justify-center">
                        <ExternalLink className="w-12 h-12 text-[#B0AFAF] opacity-50" />
                      </div>
                    )}
                  ...
                  <div className="p-4">
                    ...
                    {item.description && (
                      <p className="text-sm text-[#B0AFAF] mt-1 line-clamp-2">
                        {item.description}
                      </p>
                    )}
                    <div className="flex items-center justify-between mt-3">
                      ...
                      {item.media_type === "link" && (
                        <ExternalLink className="w-3 h-3 text-[#B0AFAF]" />
                      )}
                    </div>
                  </div>
                </div>
          ...
          <div className="mt-6 pt-6 border-t border-[#343434]">
            <div className="grid grid-cols-3 gap-4 text-center">
              <div>
                ...
                <div className="text-sm text-[#B0AFAF]">Videos</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-white">
                  {media.filter((m) => m.media_type === "image").length}
                </div>
                <div className="text-sm text-[#B0AFAF]">Images</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-white">
                  {media.filter((m) => m.media_type === "link").length}
                </div>
                <div className="text-sm text-[#B0AFAF]">Links</div>
              </div>
            </div>
          </div>
        </CardContent>
      ...
      {selectedMedia && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#1A1A1A] rounded-2xl border border-[#343434] max-w-4xl w-full max-h-[90vh] overflow-hidden">
            <div className="p-4 border-b border-[#343434] flex items-center justify-between">
