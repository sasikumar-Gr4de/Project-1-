      ...
      <Card className="bg-(--surface-1) border-(--surface-2)">
        ...
        <CardContent>
          <div className="text-center py-12">
            <TrendingUp className="w-16 h-16 text-(--muted-text) mx-auto mb-4 opacity-50" />
      ...
      <CardContent>
        <div className="space-y-4">
          {displayItems.map((event, index) => (
            <div
              key={index}
              className="flex items-start space-x-4 p-4 border rounded-xl bg-(--surface-0) border-(--surface-2) hover:border-primary/30 transition-all duration-300 group"
            >
              {/* Timeline line */}
              <div className="flex flex-col items-center">
                ...
                {index < displayItems.length - 1 && (
                  <div className="flex-1 w-0.5 bg-(--surface-2) mt-2"></div>
                )}
              </div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    {event.data?.competition && (
                      <Badge
                        variant="outline"
                        className="mt-2 bg-(--surface-2) text-(--muted-text) border-(--surface-2) text-xs"
                      >
                        {event.data.competition}
                      </Badge>
                    )}
                ...
                {event.type === "metric" && event.data && (
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-3 pt-3 border-t border-(--surface-2)">
                    <div className="text-center">
                      <div className="text-lg font-bold text-white">
                        {event.data.minutes || "--"}
                      </div>
                      <div className="text-xs text-(--muted-text)">Minutes</div>
                    </div>
                    <div className="text-center">
                      <div className="text-lg font-bold text-white">
                    ...
                    <div className="text-center">
                      ...
                      <div className="text-xs text-(--muted-text)">Passes</div>
                    </div>
                    <div className="text-center">
                      <div className="text-lg font-bold text-white">
                        {event.data.source
                      ...
                      <div className="text-xs text-(--muted-text)">Source</div>
                    </div>
                  </div>
                )}
              </div>
        ...
        {displayTimeline.length > 5 && (
          <div className="flex justify-center mt-6">
            <Button
              ...
              className="bg-(--surface-0) border-(--surface-2) text-foreground hover:bg-(--surface-2) hover:border-primary transition-all duration-300"
            >
              {expanded ? (
                <>
