...
const PassportHeader = ({ passport, player }) => {
  ...
  const getVerificationColor = () => {
    switch (verificationBadge?.status) {
      ...
      default:
        return "bg-[var(--surface-2)] text-[var(--muted-text)]";
    }
  };
  ...
  return (
    <Card className="bg-[var(--surface-1)] border-[var(--surface-2)] hover:border-primary/30 transition-all duration-300">
      <CardContent className="p-6">
        <div className="flex flex-col lg:flex-row items-start lg:items-center space-y-6 lg:space-y-0 lg:space-x-6">
          {/* Headshot */}
          <div className="shrink-0">
            <div className="w-24 h-24 lg:w-32 lg:h-32 rounded-2xl border-4 border-primary/20 bg-[var(--surface-0)] overflow-hidden shadow-lg">
              {identity?.headshot_url ? (
                ...
                <div className="w-full h-full bg-linear-to-br from-primary to-[var(--accent-2)] flex items-center justify-center">
                  <User className="w-12 h-12 text-[var(--ink)]" />
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between space-y-3 lg:space-y-0">
              ...
              {/* Season Info */}
              <div className="bg-[var(--surface-0)] border border-[var(--surface-2)] rounded-xl p-3 min-w-48">
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-[var(--muted-text)]">Season</span>
                    <span className="text-white font-semibold text-sm">
                      {passportData?.season || "2024-25"}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-[var(--muted-text)]">Squad Level</span>
                    <span className="text-white font-semibold text-sm">
            ...
            {(identity?.height_cm || identity?.weight_kg) && (
              <div className="flex items-center space-x-6 mt-4 pt-4 border-t border-[var(--surface-2)]">
                {identity.height_cm && (
                  <div className="text-center">
                    <div className="text-2xl font-bold text-white">
                      {identity.height_cm}cm
                    </div>
                    <div className="text-xs text-[var(--muted-text)]">Height</div>
                  </div>
                )}
                {identity.weight_kg && (
                  <div className="text-center">
                    ...
                    <div className="text-xs text-[var(--muted-text)]">Weight</div>
                  </div>
                )}
                {identity.height_cm && identity.weight_kg && (
                  <div className="text-center">
                    ...
                    <div className="text-xs text-[var(--muted-text)]">BMI</div>
                  </div>
                )}
              </div>
            )}
