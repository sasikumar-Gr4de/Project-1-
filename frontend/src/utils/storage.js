...
const ENCRYPTION_KEY =
  import.meta.env.VITE_ENCRYPTION_KEY || "DEFAULT_ENCRYPTION_KEY_12345";
...
export const migrateToEncryptedStorage = (keyPatterns = []) => {
  const migrated = [];
  ...
  try {
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (shouldMigrate) {
        ...
        if (isLikelyEncrypted(value)) continue;

        // Migrate to encrypted storage
        if (encryptedStorage.setItem(key, value)) {
          migrated.push(key);
        }
      }
    }
    ...
    console.log(
      `Migrated ${migrated.length} items to encrypted storage:`,
      migrated
    );
    return migrated;
  } catch (error) {
